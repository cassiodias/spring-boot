 ...
 * @author Dave Syer
 */
public class CustomLayoutRootTests {
	...
	public static void createProject() throws IOException {
		project = new ProjectCreator().createProject("custom-layout-root");
	}
	...
	public void customLayout() throws Exception {
		...
		File buildLibs = new File("target/custom-layout-root/build/libs");
		File executableJar = new File(buildLibs, "custom-layout-root.jar");
		...
		ProcessBuilder builder = new ProcessBuilder(javaCommand(),
				"-Dloader.parent.boot=true", "-Dloader.parent.first=false", "-jar",
				executableJar.getAbsolutePath());
		builder.redirectErrorStream(true);
		File file = new File("target/custom-layout-root/build/out.log");
